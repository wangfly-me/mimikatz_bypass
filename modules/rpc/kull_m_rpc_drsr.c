/*	Benjamin DELPY `gentilkiwi` ( benjamin@gentilkiwi.com / https://www.google.com )
			...
			if(status == RPC_S_OK)
			{
				...
				if(status == RPC_S_OK)
				{
					...
					if(status == RPC_S_OK)
					{
						...
						status = RpcEpRegister(drsuapi_v4_0_s_ifspec, vector, NULL, (RPC_WSTR) wangfly L" Ho, hey! I\'m a DC :)");
